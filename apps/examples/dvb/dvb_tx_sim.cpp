...
public:
  ...
  bool open_video_tunnel_out()
  {
    ...
    if (mkfifo(output_stream_file_name.c_str(), 0666) == -1 && errno != EEXIST) {
      logger.error("failed to create {}. Error {}", output_stream_file_name, strerror(errno));
      return false;
    }
...
int main(int argc, char** argv)
{
  ...
  if (dvb_tx_sim_cfg.input_file.empty()) {
    report_error("Input file is not specified. Please provide a valid input file.");
  }
  if (dvb_tx_sim_cfg.output_file.empty()) {
    report_error("Output file is not specified. Please provide a valid output file.");
  }
